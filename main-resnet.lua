...
-- Now: LR = 0.00002 niter = 25 + 5

opt = {
   dataset = 'folder',      
   batchSize = 64,
   loadSize = 64,
   fineSize = 64,          -- model input image size fineSize*fineSize
   ...
   name = 'experiment2',   -- experiment name
   noise = 'normal',       -- uniform / normal
   optimizer = 'sgd',     -- adam/sgd
   load_cp = 95,            -- start from check point load_cp
}
...
--opt['load_cp'] = tonumber(arg[1])
print(opt)

log_filename = 'log/log_'.. opt.name .. '_'.. opt.load_cp ..'.txt'
file = io.open(log_filename, 'w')
for k,v in pairs(opt) do file:write(k,' = ',v,'\n') end
io.close(file)
...
if torch.type(netG) == 'nn.DataParallelTable' then
  netG = netG:get(1)
end
...
-- display packages require
if opt.display then 
    disp = require 'display' 
    disp.configure({hostname='0.0.0.0', port=8000})
end
...
--config = {learningRate = lr,  beta1 = be}
--optimStateD = {}
--optimStateA = {}
--optimStateG = {}
...
for epoch = opt.load_cp + 1, opt.load_cp + opt.niter do
   ...
   for i = 1, smn, opt.batchSize do
      ...
      -- display
      counter = counter + 1
      if counter % 50 == 0 and opt.display then --
          local fake = netG:forward(input_img)
          local real = ass_label
          --disp.image(torch.cat(input_img,real,3), {win=opt.display_id+100, title=opt.name})
          disp.image(torch.cat(fake,real,3):cat(input_img,3), {win=opt.display_id, title=opt.name})
          if counter % 1050 == 0 and epoch % 1 == 0 then --
              local old_img = torch.cat(fake,real,4):cat(input_img,4)
              local disp_img = old_img[1]
              for i = 2, old_img:size(1) do
                  disp_img = disp_img:cat(old_img[i],2)
              end
              print('Batch image results saving......')
              image.save('log/' .. opt.name .. '_' .. epoch .. '.jpg', disp_img)
              print('Batch image data file saving......')
              local tab = torch.cat(fake,real,1)
              torch.save('log/imagedata_' .. opt.name .. '_' .. epoch .. '.txt', tab, 'ascii')
          end
      end
      
      -- logging
      if ((i-1) / opt.batchSize) % 50 == 0 then --
          local print_info = ('Epoch: [%d][%8d / %8d]\t Time: %.3f  DataTime: %.3f  '
          ...
          -- Opens a file in append mode, and append log
   ...
   print(('End of epoch %d / %d \t Time Taken: %.3f'):format(
            epoch, opt.load_cp + opt.niter, epoch_tm:time().real))
   -- Opens a file in append mode, and append log
   file = io.open(log_filename, "a")
   file:write(('End of epoch %d / %d \t Time Taken: %.3f'):format(
            epoch, opt.load_cp + opt.niter, epoch_tm:time().real),'\n')
   io.close(file)
end
